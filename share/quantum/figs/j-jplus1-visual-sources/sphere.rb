...
$resolution = 30 # a multiplier; typical values are 1 for draft, 5 for medium resolution, 30 for high resolution
...
def f(theta,phi)
  ...
  # Ylm
  z1 = ylm(1,-1,theta,phi+3.3)
  z2 = ylm(1,1,theta,phi+3.3)
  re = z1[0]*Math::cos(z1[1])+z2[0]*Math::cos(z2[1])
  im = z1[0]*Math::sin(z1[1])+z2[0]*Math::sin(z2[1])
  z = [0,0]
  z[0] = Math::sqrt(re**2+im**2)
  z[1] = Math::atan2(im,re)
  z[0] = z[0]/Math::sqrt(2)
  return z
...
def ylm(l,m,theta,phi)
  ...
  mag = GSL::Sf::legendre_Plm(l,m.abs,Math::cos(theta))
  arg = put_in_range(m*phi,2.0*$PI)
  return clean_up_complex([mag,arg])
end
