<% begin_sec("Time Is Not Absolute") %>\label{sec:time-not-absolute}
...
<% begin_sec("The correspondence principle") %>
...
<%
  fig(   
    ...
    %q{%
       ...
       m4_ifelse(__me,1,[::],[:Cf.~problem \ref{hw:pound-rebka}, p.~\pageref{hw:pound-rebka}.:])
    },
    {
...
<% end_sec %> % Time Is Not Absolute

<% begin_sec("Distortion of Space and Time") %>\label{sec:x-t-distortion}
...
a unitless 1? But remember that this is expressed in our special relativistic units, in which the same units are used for
...
an extra conversion factor $c$, and the equation becomes
\begin{equation*}
...
<% begin_sec("Velocities don't simply add and subtract.") %>\label{relativistic-combination-of-vel}
m4_ifelse(__me,1,[: %
...
<% begin_sec("A universal speed limit") %>
